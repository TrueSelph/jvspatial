...
1. **When indexes are created**: `GraphContext.ensure_indexes()` is invoked when a class is first used for `save` / `find` (unless `JVSPATIAL_AUTO_CREATE_INDEXES` is false, or in serverless mode where the default is off to reduce cold start). The **jvagent** server and `jvagent bootstrap` run `run_index_migration()` from `jvagent.core.index_bootstrap` at startup, which eagerly calls `ensure_indexes` for all core entity types before traffic, so most deployments build or migrate indexes before the first user request.
...
4. **Custom adapters**: Implement `create_index` for performance; implement `drop_deprecated_indexes` if your backend has named indexes and you want jvagent’s startup list of obsolete names to take effect. See the [Custom Database guide](custom-database-guide.md#database-interface).
