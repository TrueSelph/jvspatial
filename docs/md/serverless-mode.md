...
- **Auth**: unauthenticated by default. If **`JVSPATIAL_DEFERRED_INVOKE_SECRET`** is set, each request must send that value in **`X-JVSPATIAL-Deferred-Authorize`** or **`Authorization: Bearer <secret>`**; otherwise the route returns 401. Still prefer private network / VPC boundaries for production.
- **Environment defaults (best-effort)**: when `is_serverless_mode()` is true and `detect_serverless_provider() == "aws"`, `Server.__init__` calls **`apply_aws_lwa_env_defaults()`** (`jvspatial.runtime.lwa`), which uses `os.environ.setdefault` for:
  - `AWS_LWA_PASS_THROUGH_PATH` → the same path as `APIRoutes.deferred_invoke_full_path()`
