...
from jvspatial.core.entities import Node, Edge, Walker
...
class User(Node):
    ...
    async def validate_custom(self) -> bool:
        ...
        if not self.name:
            raise ValidationError("Name cannot be empty", field_errors={"name": "Required field"})
        
        if self.age < 0:
            raise ValidationError("Age cannot be negative", field_errors={"age": "Must be positive"})
        
        if "@" not in self.email:
            raise ValidationError("Invalid email format", field_errors={"email": "Must contain @"})
...
async def demonstrate_walker_error_handling():
    ...
    try:
        ...
        users = [
            ...
            await User.create(name="BadUser", email="bad@example.com", age=35),  # Will cause error
        ]
        
        # Create connections
        connection = await Connection.create(
            left=users[0], right=users[1], 
        ...
        try:
            ...
            # Show error details
            errors = [item for item in report if isinstance(item, dict) and "error_type" in item]
            for error in errors:
