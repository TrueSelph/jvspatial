...
from .entities import ScheduleConfig, ScheduledTask
...
def register_scheduled_tasks(scheduler: SchedulerService) -> None:
...
def on_schedule(
    ...
    def decorator(func: Callable) -> Callable:
        ...
        # If a specific scheduler was provided, register immediately
        target_scheduler = scheduler or _default_scheduler
        if target_scheduler:
            try:
                target_scheduler.register_task(scheduled_task)
                logging.getLogger(__name__).debug(
                    f"Immediately registered scheduled task: {actual_task_id}"
                )
            except Exception as e:
                logging.getLogger(__name__).error(
                    f"Failed to immediately register scheduled task {actual_task_id}: {e}"
                )
