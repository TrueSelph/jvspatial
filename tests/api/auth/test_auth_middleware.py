...
class TestAuthConfig:
    ...
    async def test_configure_auth(self):
...
class TestRateLimiter:
    ...
    async def test_is_allowed_different_users(self):
...
class TestAuthenticationMiddleware:
    ...
    async def test_rate_limiting(self):
        ...
        request = MagicMock(spec=Request)
        request.url.path = "/api/protected"
        request.state = SimpleNamespace(endpoint_auth=True, current_user=self.test_user)
    ...
    async def test_permission_checking(self):
        ...
        request.state = SimpleNamespace(
            endpoint_auth=True,
            required_permissions=["write_data"],
            current_user=self.test_user,
    ...
    async def test_role_checking(self):
        ...
        request.url.path = "/api/protected"
        request.state = SimpleNamespace(
            endpoint_auth=True, required_roles=["admin"], current_user=self.test_user
    ...
    async def test_authenticate_jwt_success(self):
        ...
        with patch(
            ...
            with patch(
                ...
                # Mock database context for session validation
                with patch(
                    ...
                    # Mock session data - session exists and is active
    ...
    @pytest.mark.asyncio
    async def test_authenticate_jwt_session_validation(self):
        """Test JWT authentication with session validation."""
        request = MagicMock(spec=Request)
        request.headers.get.return_value = "Bearer valid_token"
...
class TestAuthUtilities:
    ...
    async def test_authenticate_user_by_email(self):
        ...
        with patch("jvspatial.api.auth.entities.User.find_by_email", return_value=None):
            with patch(
                "jvspatial.api.auth.entities.User.find_by_email",
                return_value=self.test_user,
            ):
