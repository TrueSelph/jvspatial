...
class TestJsonDBOperations:
    ...
    async def test_delete_existing_document(self):
        ...
        test_data = {"id": "test_doc_3", "name": "TestDocument", "context": {}}
...
class TestJsonDBFind:
    ...
    async def test_find_with_nested_query(self):
        ...
        results = await self.db.find(
            "test_collection", {"context.nested.field": "value1"}
        )
    ...
    async def test_find_with_in_operator(self):
        ...
        # Test $in operator
        results = await self.db.find(
            "test_collection", {"context.category": {"$in": ["A", "C"]}}
        )
        assert len(results) == 2
    ...
    async def test_find_with_nin_operator(self):
        ...
        # Test $nin operator
        results = await self.db.find(
            "test_collection", {"context.status": {"$nin": ["inactive", "pending"]}}
        )
        assert len(results) == 2
