...
from jvspatial.core.entities import Edge, Node, find_subclass_by_name
...
class TestEdgeProperties:
    ...
    async def test_railroad_properties(self):
        ...
        railroad = Railroad(left=node_a, right=node_b, electrified=True, gauge="broad")
...
class TestEdgePersistence:
    ...
    async def test_edge_save_and_retrieve(self):
        ...
        original = Highway(
            left=node_a, right=node_b, lanes=4, speed_limit=65, distance_km=100.0
        )
        await original.save()
...
class TestEdgeLegacyFormat:
    ...
    async def test_legacy_data_format_retrieval(self):
        ...
        legacy_data = {
            "id": "e:Edge:legacy123",
            "name": "Edge",
            "context": {"source": node_a.id, "target": node_b.id, "direction": "out"},
        }
...
class TestEdgeValidation:
    ...
    async def test_edge_with_mixed_parameters(self):
        ...
        edge = Edge(
            left=node_a, right=node_b, source="custom_source", target="custom_target"
        )
